...
public:
  ...
  constexpr explicit modint(uint_type const v)
    : v{(sint_type(v) >= mod ? sint_type(v) : sint_type(v) % mod)} {} // special
  // assignment op
...
#pragma region friend
  // equality compare
  friend constexpr bool operator==(modint const& lhs, modint const& rhs) noexcept {
    return lhs.v == rhs.v;
  }
  friend constexpr bool operator==(modint const& lhs, sint_type const& rhs) noexcept {
    return lhs.v == rhs;
  }
  friend constexpr bool operator==(sint_type const& lhs, modint const& rhs) noexcept {
    return rhs == lhs;
  }
  friend constexpr bool operator!=(modint const& lhs, modint const& rhs) noexcept {
    return !(lhs == rhs);
  ...
  friend auto& operator<<(std::basic_ostream<CharT, Traits>& os, modint const& rhs) {
    return os << rhs.v;
  }
  template <typename CharT, typename Traits>
