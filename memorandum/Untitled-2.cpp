...
namespace ns {
  ...
  template <typename Array>
  array_iterator<Array> operator+(const array_iterator<Array>& left,
                                  std::size_t right) {
    return array_iterator<Array>(left) += right;
  }
  template <typename Array>
  bool operator<(const array_iterator<Array>& left,
                 const array_iterator<Array>& right) {
  ...
  public:
    ...
    const Array& a;
    std::size_t i;
    array_const_iterator(const Array& a, std::size_t i) : a(a), i(i) {}
  ...
  template <typename Array>
  array_const_iterator<Array> operator+(const array_const_iterator<Array>& left,
                                        std::size_t right) {
    return array_const_iterator<Array>(left) += right;
  }
  template <typename Array>
  bool operator<(const array_const_iterator<Array>& left,
                 const array_const_iterator<Array>& right) {
    return left.i < right.i;
  }
  template <typename Array>
  bool operator>(const array_const_iterator<Array>& left,
  ...
  class array {
  public:
    // クラステンプレート内でarrayはarray<T, N>と同じ
    using self            = array;
    using value_type      = T;
