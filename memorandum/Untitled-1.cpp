...
public:
  ...
  auto operator-() const& { return fractional(-num, denom); }
  auto operator-() && {
  ...
  auto& operator+=(const fractional& other) & {
    ...
    denom =
      new_denom(num * other.denom + other.num * denom, denom * other.denom);
    return *this;
  }
  auto operator+=(const fractional& other) && {
    auto result = move(*this);
    result.num  = new_num(result.num * other.denom + other.num * result.denom,
                         result.denom * other.denom);
    result.denom =
      new_denom(result.num * other.denom + other.num * result.denom,
                result.denom * other.denom);
    return result;
  }
  void print() const { cout << "(" << num << "," << denom << ") "; }
};
...
// overload解決のため必要
auto operator+(fractional&& x, fractional&& y) { return move(x) + y; }
void print(const fractional& x) { x.print(); }
...
int main() { fractional a = f(fractional(1, 2)); }
