...
struct range: std::pair<Itr,Itr>{
  ...
  range(); range(const range&)=default; range(range&&)=default;
...
template<typename R>
auto make_range(R&& r){
  return range<decltype(begin(std::forward<R>(r)))>(begin(std::forward<R>(r)),end(std::forward<R>(r)));}
template<typename Itr>
...
namespace detail{\
  class CONCAT(target,_tag0){\
    ...
    ->decltype(source(std::forward<R>(r))){ return source(std::forward<R>(r)); }\
    template<typename R>\
    auto operator()(const R& r) const\
    ->decltype(source(r)){ return source(r); }};\
  template<typename R>\
  auto operator|(R&& r, const CONCAT(target,_tag0)& tag)\
  ->decltype(tag(std::forward<R>(r))){ return tag(std::forward<R>(r)); }\
  template<typename R>\
  auto operator|(const R& r, const CONCAT(target,_tag0)& tag)\
  ->decltype(tag(r)){ return tag(r); }}\
...
namespace detail{\
  ...
  class CONCAT(target,_tag1){\
    ...
    template<typename R>\
    auto operator()(R&& r) const\
    ->decltype(source(std::forward<R>(r),x1)){ return source(std::forward<R>(r),x1); }\
    template<typename R>\
    auto operator()(const R& r) const\
    ->decltype(source(r,x1)){ return source(r,x1); }};\
  ...
  ->decltype(tag(r)){ return tag(r); }}\
template<typename T1>\
auto target(const T1& x1){ return detail::CONCAT(target,_tag1)<T1>(x1); }
...
namespace detail{\
  ...
  class CONCAT(target,_tag2){\
    T1 x1; T2 x2;\
    public:\
    CONCAT(target,_tag2)(const T1& x1_, const T2& x2_): x1(x1_),x2(x2_){}\
    template<typename R>\
    auto operator()(R&& r) const\
    ->decltype(source(std::forward<R>(r),x1,x2)){ return source(std::forward<R>(r),x1,x2); }\
    template<typename R>\
    auto operator()(const R& r) const\
    ->decltype(source(r,x1,x2)){ return source(r,x1,x2); }};\
  ...
  ->decltype(tag(r)){ return tag(r); }}\
template<typename T1, typename T2>\
auto target(const T1& x1, const T2& x2){ return detail::CONCAT(target,_tag2)<T1,T2>(x1,x2); }
...
namespace detail{\
  ...
  class CONCAT(target,_tag3){\
    T1 x1; T2 x2; T3 x3;\
    public:\
    CONCAT(target,_tag3)(const T1& x1_, const T2& x2_, const T3& x3_): x1(x1_),x2(x2_),x3(x3_){}\
    template<typename R>\
    auto operator()(R&& r) const\
    ->decltype(source(std::forward<R>(r),x1,x2,x3)){ return source(std::forward<R>(r),x1,x2,x3); }\
    template<typename R>\
    auto operator()(const R& r) const\
    ->decltype(source(r,x1,x2,x3)){ return source(r,x1,x2,x3); }};\
  template<typename R, typename T1, typename T2, typename T3>\
  auto operator|(R&& r, const CONCAT(target,_tag3)<T1,T2,T3>& tag)\
  ->decltype(tag(std::forward<R>(r))){ return tag(std::forward<R>(r)); }\
  template<typename R, typename T1, typename T2, typename T3>\
  auto operator|(const R& r, const CONCAT(target,_tag3)<T1,T2,T3>& tag)\
  ->decltype(tag(r)){ return tag(r); }}\
template<typename T1, typename T2, typename T3>\
auto target(const T1& x1, const T2& x2, const T3& x3){ return detail::CONCAT(target,_tag3)<T1,T2,T3>(x1,x2,x3); }
